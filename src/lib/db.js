...
export async function verifyUser(username, password) {
  ...
  if (user) {
    return {
      success: true,
      user: { username: user.username, role: user.role, mobile: user.mobile },
    };
  }
...
export async function getPackageById(id) {
  await dbConnect();
  console.log(`DB: Searching for package with id: "${id}"`);
  const pkg = await Package.findOne({ id });
  console.log(`DB: Search result:`, pkg ? 'Found' : 'Null');
  if (!pkg) return null;
...
export async function getBrochureById(id) {
  const brochures = await getBrochures();
  console.log(`DEBUG: Searching for brochure ID: "${id}"`);
  console.log(`DEBUG: Available IDs: ${brochures.map(b => b.id).join(', ')}`);
  const found = brochures.find(b => String(b.id).trim() === String(id).trim());
